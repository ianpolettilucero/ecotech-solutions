...
/** Sangría del JSON: el reporte se descarga y se lee a ojo, no se transporta. */
const SANGRIA = 2;
 ...
 * procesarlo con otra herramienta y necesita el número, no "ARS 1.234,56".
 */
export class ExportadorJSON extends Exportador {
  ...
  override async exportar(reporte: ReporteDTO): Promise<Uint8Array> {
    ...
    // el orden de claves del archivo y evita que un campo agregado mañana al
    // DTO (interno, o sensible) se filtre solo en la descarga.
    const contenido = {
      // `tipo` es parte del contrato de `ReporteDTO` y de nada sirve un archivo
      // "sin perdida" del que haya que deducir a que informe pertenece; además
      // ya viaja en el nombre del archivo, así que no revela nada nuevo.
      tipo: reporte.tipo,
